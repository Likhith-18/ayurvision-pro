    ...
    reader.onload = async () => {
      try {
        ...
        const response = await axios.post(
          `${import.meta.env.VITE_SERVER_URL}/plant`,
          { image: image }
        );
        setLoading(false);
          ...
          <div className="w-full max-w-3xl bg-white p-8 rounded shadow-md pb-16">
            {!uploaded && (
              <div className="flex items-center justify-center mb-16 mt-12">
                <h1 className="m-auto text-3xl font-semibold">
                  Hi! Upload a plant image to get it identified!
                </h1>
              </div>
            )}
            {!uploaded && (
              ...
              >
                {({ getRootProps, getInputProps }) => (
                  <section>
                    <div
                      {...getRootProps()}
                      className="h-[300px] w-5/6 flex justify-center items-center m-auto mt-5 border-2 border-dashed border-gray-400 rounded bg-gray-100"
                    >
                      <input {...getInputProps()} />
            ...
            {uploaded && (
              <img
                className="m-auto max-h-[500px] mt-8"
                src={imageUrl}
                alt="uploaded image"
              />
            )}
            {loading && (
              <div className="flex justify-center items-center my-10">
                <p className="text-3xl inline px-5">
            ...
            {gotInference && (
              <div className="flex flex-col justify-center items-center my-10 m-auto">
